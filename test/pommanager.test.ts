...
import { Pom } from "../src/managers/pom";

test('Pom Manager finds dependencies from string',async () => {
  const config = readFileSync("./test/assets/jboss.pom.xml", "utf8").toString();
...
test('Pom Manager looks up dependencies from string',async () => {
  const config = readFileSync("./test/assets/jboss.pom.xml", "utf8").toString();
